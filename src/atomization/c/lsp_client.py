...
class CAtomizer:
    ...
    def _start_clangd(self) -> None:
        ...
        print(f"Starting clangd server", file=sys.stderr)
        ...
        while time.time() - start_time < 5:  # 5 second timeout
            ...
            if stderr_line:
                print(f"clangd stderr during init: {stderr_line.strip()}", file=sys.stderr)
            
            # Check if process is still running
            if self.clangd_process.poll() is not None:
                raise RuntimeError("clangd process ended unexpectedly during initialization")
    ...
    def _read_response(self, timeout: float = 5.0) -> dict:
        ...
        while True:
            ...
            if time.time() - start_time > timeout:
                print(f"Timeout after {timeout} seconds. Process state:", file=sys.stderr)
                print(f"  Process running: {self.clangd_process.poll() is None}", file=sys.stderr)
                print(f"  Return code: {self.clangd_process.poll()}", file=sys.stderr)
                raise RuntimeError(f"Timeout waiting for response after {timeout} seconds")
            
            # Check if process is still running
            if self.clangd_process.poll() is not None:
                print("clangd process ended unexpectedly. Last stderr output:", file=sys.stderr)
            ...
            if stdout_line:
                ...
                try:
                    ...
                    if "id" in response:  # This is a response to our request
                        print(f"Got response: {json.dumps(response, indent=2)}", file=sys.stderr)
                        return response
                except json.JSONDecodeError:
                    print(f"Failed to parse JSON: {stdout_line.strip()}", file=sys.stderr)
                    continue
    ...
    def _send_lsp_request(self, method: str, params: dict) -> dict:
        ...
        if not self.clangd_process:
            ...
            print(f"Sending initialize request: {json.dumps(init_request, indent=2)}", file=sys.stderr)
            ...
            if "error" in init_response:
                print(f"Initialize error: {json.dumps(init_response['error'], indent=2)}", file=sys.stderr)
                raise RuntimeError(f"Failed to initialize clangd: {init_response['error']}")
