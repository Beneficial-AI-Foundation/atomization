...
class DafnyAtomizer(AtomizerPlugin[DafnySymbol, DafnyRef]):
    ...
    async def request_all_symbols(self) -> list[DafnySymbol]:
        ...
        self.logger.info(f"Found {len(syms)} symbols (Classes: kind 5, Methods: kind 6, Functions: kind 12)")
        return syms
    ...
    async def build_dependency_graph(self, symbols: list[DafnySymbol], source_content: str = "") -> dict[str, list[str]]:
        ...
        for symbol in symbols:
            ...
            if source_content:
                deps_in_code = self._find_dependencies_in_code(symbol, symbols, source_content)
    ...
    def _find_dependencies_in_code(self, symbol: DafnySymbol, all_symbols: list[DafnySymbol], source_content: str) -> set[str]:
    ...
    def _symbol_referenced_in_code(self, symbol_name: str, code: str) -> bool:
        ...
        for i, line in enumerate(lines):
            ...
            if i == 0 and ('function ' + symbol_name in line or 'method ' + symbol_name in line or 'class ' + symbol_name in line):
                continue
