...
def handle_method_or_function(
    lines: list[str], start_idx: int, chunk_order: int
    ...
    while i < len(lines):
        ...
        if "{" in line:
            break
        if any(
            x in line for x in ["requires", "ensures", "decreases", "reads", "modifies"]
        ):
...
def handle_class(
    ...
    while i < len(lines):
        ...
        if any(
            line.strip().startswith(x) for x in ["method", "function", "ghost", "lemma"]
        ):
            break
    ...
    while i < len(lines) and brace_count > 0:
        ...
        if line.strip().startswith(
            ...
            new_chunks, new_i, new_order = handle_method_or_function(
                lines, i, chunk_order
            )
