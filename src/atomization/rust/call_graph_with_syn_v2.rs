...
impl CallGraph {
    ...
    fn to_svg(&self, output_path: &str) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for atom in &self.atoms {
            let node = graph.add_node((atom.identifier.clone(), atom.body.clone()));
            node_indices.insert(&atom.identifier, node);
        }

        for atom in &self.atoms {
            if let Some(&source) = node_indices.get(&atom.identifier) {
                for dependency in &atom.deps {
        ...
        for node in graph.node_indices() {
            let (identifier, body) = &graph[node];
...
impl FunctionCallVisitor {
    ...
    fn process_current_element(&mut self, statement_type: &str) {
        if let Some(current_element) = &self.current_element {
            ...
            self.atoms.push(Atom {
                identifier: current_element.clone(),
    ...
    fn store_body(&mut self, identifier: &str, body: &str) {
        self.bodies.insert(identifier.to_string(), Self::clean_body(body));
    }
