...
class TestDafnyLSPSymbolKinds:
    """Test to verify what symbol kinds the Dafny LSP server returns."""
    
    def test_symbol_kinds_from_examples(self, dafny_lsp_client, dafny_example_files):
    ...
    def test_all_symbol_kinds_investigation(self, dafny_lsp_client, dafny_example_files):
        ...
        for sym in all_symbols_with_kinds:
            ...
            print(f"  {sym['name']:<20} | kind: {kind:2d} | {Path(sym['uri']).name if sym['uri'] != 'unknown' else 'unknown'}")
